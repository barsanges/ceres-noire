...
spec = do
  describe "withinRange" $ do
    ...
    it "should always fail if the inventory is empty" $ property $
      \ x -> ((withinRange Nothing (abs x) (1 + abs x) (empty 0)) `eitherShouldMatch` (Left "The problem is infeasible!"))

    it "should always fail if the minimum value is negative" $ property $
      \ x -> ((withinRange Nothing (-(abs x) - 1) (abs x) (empty 0)) `eitherShouldMatch` (Left "The minimum value should be a positive float!"))

    it "should always fail if the maximum value is lower than the minimum value" $ property $
      \ x -> ((withinRange Nothing ((abs x) + 10) ((abs x) + 5) (empty 0)) `eitherShouldMatch` (Left "The maximum value should be greater than the minimum value!"))

    it "should always fail if the maximal number of stamp is lower or equal to 0" $ property $
      \ n x -> ((withinRange (Just (-(abs n))) (abs x) (abs (2 * x)) (empty 0)) `eitherShouldMatch` (Left "The maximal number of stamps should be strictly positive!"))
